...
class MemoryStorage(StorageService):
        ...
        async with self._lock:
            try:
                ...
                MemoryStorage._scan_results = [
                    r for r in MemoryStorage._scan_results 
                ...
                if len(MemoryStorage._scan_results) > self.max_scan_results:
                    MemoryStorage._scan_results = MemoryStorage._scan_results[:self.max_scan_results]
    ...
    async def get_scan_results(
        ...
        try:
            ...
            if scan_id:
                results = [
                    copy.deepcopy(r) for r in MemoryStorage._scan_results 
    ...
    async def get_known_malicious_packages(self) -> List[MaliciousPackage]:
        ...
        try:
            packages = [
                copy.deepcopy(pkg) 
                for pkg in MemoryStorage._malicious_packages.values()
            ]
        ...
        async with self._lock:
            try:
                ...
                for package in packages:
                    ...
                    if package.package_identifier not in MemoryStorage._malicious_packages:
                        new_count += 1
                    
                    # Store a deep copy
                    MemoryStorage._malicious_packages[package.package_identifier] = copy.deepcopy(package)
                
                logger.debug(f"Successfully stored {len(packages)} malicious packages ({new_count} new)")
                return True
