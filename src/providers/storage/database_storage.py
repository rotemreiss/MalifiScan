...
class ScanResultModel(Base):
    ...
    )  # Not unique anymore, can have multiple rows per scan
    registry_id = Column(
        String, ForeignKey("registries.id"), nullable=False
    )  # Which registry was scanned
    ...
    malicious_packages = relationship(
        "MaliciousPackageModel", back_populates="scan_result"
    )
...
class MaliciousPackageModel(Base):
    ...
    blocked_packages = relationship(
        "BlockedPackageModel", back_populates="malicious_package"
    )
...
class FindingModel(Base):
    ...
    id = Column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    scan_result_id = Column(String, ForeignKey("scan_results.id"), nullable=False)
    malicious_package_id = Column(
        String, ForeignKey("malicious_packages.id"), nullable=False
    )
...
class BlockedPackageModel(Base):
    ...
    )  # e.g., 'blocked', 'quarantined', 'deleted'
    block_status = Column(
        String, nullable=False
    ...
    malicious_package = relationship(
        "MaliciousPackageModel", back_populates="blocked_packages"
    )
...
class DatabaseStorage(StorageService):
    ...
    def __init__(
        ...
        self.default_registry_url = (
            default_registry_url or "https://default-registry.example.com"
        )
    ...
    def _ensure_default_registry(self) -> str:
        ...
        with self.SessionLocal() as session:
            registry = (
                session.query(RegistryModel)
                .filter_by(base_url=self.default_registry_url)
                .first()
            )
            if not registry:
                registry = RegistryModel(
                    base_url=self.default_registry_url, type="jfrog"
                )
    ...
    async def create_registry(
        self, url: str, registry_type: str, name: str = None, description: str = None
    ) -> str:
        """
        ...
        try:
            with self.SessionLocal() as session:
                ...
                registry = RegistryModel(base_url=url, type=registry_type)
                session.add(registry)
                session.commit()
        ...
        try:
            with self.SessionLocal() as session:
                ...
                if registry_service and hasattr(registry_service, "base_url"):
                    ...
                    registry = (
                        session.query(RegistryModel)
                        .filter_by(base_url=registry_service.base_url)
                        .first()
                    )
                    if not registry:
                    ...
                    registry = (
                        session.query(RegistryModel).filter_by(type="default").first()
                    )
                    if not registry:
                        registry = RegistryModel(
                            type="default",
                ...
                existing = (
                    ...
                    .filter_by(scan_id=scan_result.scan_id)
                    .first()
                )
                ...
                if existing:
                    ...
                    existing.execution_duration_seconds = (
                        scan_result.execution_duration_seconds
                    )

                    # Clear existing malicious packages and findings
                    session.query(MaliciousPackageModel).filter_by(
                        scan_result_id=existing.id
                    ).delete()
                    session.query(FindingModel).filter_by(
                        scan_result_id=existing.id
                    ).delete()
                    session.query(BlockedPackageModel).filter_by(
                        scan_result_id=existing.id
                    ).delete()
                ...
                for package in scan_result.malicious_packages_found:
                    ...
                    malicious_package_map[package.package_identifier] = (
                        malicious_package_model.id
                    )
                ...
                for package in scan_result.malicious_packages_list:
                    ...
                    malicious_package_id = malicious_package_map.get(
                ...
                for result in results:
                    try:
                        if result.scan_id not in scan_results_dict:
                            ...
                            blocked_packages = (
                                session.query(BlockedPackageModel)
                                .join(
                                    MaliciousPackageModel,
                                    BlockedPackageModel.malicious_package_id
                                    == MaliciousPackageModel.id,
                                )
                                .filter(BlockedPackageModel.scan_result_id == result.id)
                                .all()
                            )

                            blocked_package_names = [
                                bp.malicious_package.name for bp in blocked_packages
                            ]
                        ...
                        for mp_model in result.malicious_packages:
                            malicious_package = self._malicious_package_model_to_entity(
                                mp_model
                            )
                            scan_results_dict[
                                result.scan_id
                            ].malicious_packages_found.append(malicious_package)

                        # Add findings to malicious_packages_list
                        for finding_model in result.findings:
                            finding_package = self._finding_model_to_malicious_package(
                                finding_model
                            )
                            scan_results_dict[
                                result.scan_id
                            ].malicious_packages_list.append(finding_package)

                    except Exception as e:
                        logger.warning(
                            f"Failed to parse scan result {result.scan_id}: {e}"
                        )
                        continue
    ...
    async def get_known_malicious_packages(self) -> List[MaliciousPackage]:
        ...
        try:
            ...
            raise StorageError(
                f"Unexpected error retrieving known malicious packages: {e}"
            ) from e
    ...
    def _finding_model_to_malicious_package(
        self, model: FindingModel
    ) -> MaliciousPackage:
