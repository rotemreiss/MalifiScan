...
class CompositeNotifier(NotificationService):
    ...
    async def send_notification(self, event: NotificationEvent) -> bool:
        ...
        """
        logger.info(f"Sending notification to {len(self.notifiers)} services: {event.event_id}")
        ...
        if success:
            logger.info(f"Successfully sent notification to {successful_count}/{len(self.notifiers)} services")
        else:
            logger.error("Failed to send notification to any service")
        
        return success
    
    async def _send_to_notifier(self, notifier: NotificationService, event: NotificationEvent) -> bool:
    ...
    async def health_check(self) -> bool:
        ...
        logger.debug(f"Health check: {healthy_count}/{len(self.notifiers)} services healthy")
