...
class ProactiveSecurityUseCase:
    ...
    async def block_recent_malicious_packages(
        ...
        """
        try:
            self.logger.debug(f"Starting proactive blocking for {ecosystem} packages from last {hours} hours")
            ...
            self.logger.debug(f"Found {len(malicious_packages)} malicious {ecosystem} packages to process")
            ...
            for i, package in enumerate(malicious_packages):
                try:
                    ...
                    if block_result:
                        ...
                        self.logger.debug(f"Successfully blocked package: {package.name}")
                    else:
            ...
            self.logger.info(f"Proactive blocking complete: {success_count}/{total_processed} packages blocked")
    ...
    async def unblock_packages_by_criteria(
        ...
        """
        try:
            self.logger.debug(f"Starting bulk unblock operation for {ecosystem} (dry_run={dry_run})")
            ...
            if package_pattern:
                import fnmatch
                matching_packages = [
                    pkg for pkg in blocked_packages 
            ...
            for package_info in matching_packages:
                try:
                    ...
                    if unblock_result:
                        ...
                        self.logger.debug(f"Successfully unblocked package: {package_name}")
                    else:
