...
from .config import Config, ConfigError, ConfigLoader
...
class SecurityScannerApp:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            config_loader = ConfigLoader(
                self.config_file, self.env_file, local_config_file
            )
            ...
            self.logger.info(
                f"Application initialized in {self.config.environment} environment"
            )
            ...
            self.health_management = HealthManagementUseCase(services=self.services)
    ...
    async def run_single_scan(self) -> bool:
        ...
        try:
            ...
            if scan_result.is_successful:
                ...
                self.logger.info(
                    f"Malicious packages found: {len(scan_result.malicious_packages_found)}"
                )
                self.logger.info(
                    f"Packages blocked: {len(scan_result.packages_blocked)}"
                )
                self.logger.info(f"New threats: {scan_result.new_threats_count}")
                return True
            else:
                self.logger.error(
                    f"Scan failed with status: {scan_result.status.value}"
                )
    ...
    async def security_crossref_analysis(
        self,
        hours: int = 6,
        ...
        return await self.security_analysis.crossref_analysis(
    ...
    async def block_package_in_registry(
        ...
        return await self.package_management.block_package(
            package_name, ecosystem, version
        )

    async def search_package_in_registry(
    ...
    async def get_recent_scan_summaries(self, limit: int = 3) -> dict:
        ...
        try:
            ...
            return {"success": False, "error": str(e), "summaries": [], "count": 0}
...
async def main() -> int:
    ...
    parser.add_argument(
        "--scan", action="store_true", help="Run a single scan and exit"
    )
