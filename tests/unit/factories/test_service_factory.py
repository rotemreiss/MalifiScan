...
from src.core.interfaces import PackagesFeed, PackagesRegistryService, NotificationService, StorageService
...
class TestServiceFactory:
    ...
    def test_create_packages_feed_creation_error(self, mock_osv_feed, service_factory):
        ...
        assert "Failed to create packages feed: Provider creation failed" in str(exc_info.value)

    @patch('src.factories.service_factory.JFrogRegistry')
    def test_create_packages_registry_jfrog_success(self, mock_jfrog_registry, service_factory):
    ...
    @patch('src.factories.service_factory.JFrogRegistry')
    def test_create_packages_registry_creation_error(self, mock_jfrog_registry, service_factory):
        ...
        assert "Failed to create packages registry: Registry creation failed" in str(exc_info.value)
    ...
    def test_create_storage_service_file_success(self, mock_file_storage, service_factory):
    ...
    def test_create_storage_service_database_success(self, mock_db_storage, service_factory):
    ...
    @patch('src.factories.service_factory.DatabaseStorage')
    def test_create_storage_service_sqlite_success(self, mock_db_storage, service_factory):
    ...
    @patch('src.factories.service_factory.FileStorage')
    def test_create_storage_service_creation_error(self, mock_file_storage, service_factory):
        ...
        assert "Failed to create storage service: Storage creation failed" in str(exc_info.value)

    def test_create_null_notifier(self, service_factory):
        """Test creation of null notifier."""
        with patch('src.providers.notifications.null_notifier.NullNotifier') as mock_null_notifier:
            mock_instance = MagicMock()
            mock_null_notifier.return_value = mock_instance
            
            result = service_factory._create_null_notifier()
            
            assert result == mock_instance
            mock_null_notifier.assert_called_once()

    def test_create_null_registry(self, service_factory):
        """Test creation of null registry."""
        with patch('src.providers.registries.null_registry.NullRegistry') as mock_null_registry:
            mock_instance = MagicMock()
            mock_null_registry.return_value = mock_instance
            
            result = service_factory._create_null_registry()
            
            assert result == mock_instance
            mock_null_registry.assert_called_once()
