...
class TestPackageManagementUseCase:
    ...
    async def test_logging_calls(self, mock_registry_service, mock_storage_service):
        ...
        with patch('src.core.usecases.package_management.logging.getLogger') as mock_get_logger:
            ...
            mock_logger.debug.assert_any_call(f"Searching for package: {package_name} ({ecosystem})")
            mock_logger.debug.assert_any_call(f"Package search completed: 0 results found, blocked: False")
            ...
            mock_logger.debug.assert_any_call(f"Blocking package: {package_name} ({ecosystem}) version 1.0.0")
