...
class TestPackageManagementUseCase:
    ...
    def mock_logger(self):
        ...
        with patch(
            "src.core.usecases.package_management.logging.getLogger"
        ) as mock_get_logger:
            mock_logger = Mock()
    ...
    @pytest.mark.asyncio
    async def test_search_package_successful(
        self, use_case, mock_registry_service, mock_logger
    ):
        ...
        mock_registry_service.search_packages.assert_called_once_with(
    ...
    @pytest.mark.asyncio
    async def test_search_package_default_ecosystem(
        self, use_case, mock_registry_service, mock_logger
    ):
    ...
    @pytest.mark.asyncio
    async def test_search_package_blocked(
        self, use_case, mock_registry_service, mock_logger
    ):
    ...
    @pytest.mark.asyncio
    async def test_search_package_empty_results(
        self, use_case, mock_registry_service, mock_logger
    ):
    ...
    @pytest.mark.asyncio
    async def test_search_package_multiple_results(
        self, use_case, mock_registry_service, mock_logger
    ):
    ...
    @pytest.mark.asyncio
    async def test_search_package_different_ecosystems(
        self, use_case, mock_registry_service, mock_logger
    ):
    ...
    @pytest.mark.asyncio
    async def test_search_package_exception_during_search(
        self, use_case, mock_registry_service, mock_logger
    ):
    ...
    @pytest.mark.asyncio
    async def test_search_package_exception_during_is_blocked_check(
        self, use_case, mock_registry_service, mock_logger
    ):
        ...
        mock_registry_service.is_package_blocked.side_effect = Exception(
            "Block check failed"
        )
    ...
    @pytest.mark.asyncio
    async def test_search_package_cleanup_exception_ignored(
        self, use_case, mock_registry_service, mock_logger
    ):
    ...
    @pytest.mark.asyncio
    async def test_search_package_malicious_package_creation(
        self, use_case, mock_registry_service, mock_logger
    ):
    ...
    @pytest.mark.asyncio
    async def test_block_package_successful(
        self, use_case, mock_registry_service, mock_storage_service, mock_logger
    ):
    ...
    @pytest.mark.asyncio
    async def test_block_package_default_parameters(
        self, use_case, mock_registry_service, mock_storage_service, mock_logger
    ):
    ...
    @pytest.mark.asyncio
    async def test_block_package_wildcard_version(
        self, use_case, mock_registry_service, mock_storage_service, mock_logger
    ):
    ...
    @pytest.mark.asyncio
    async def test_block_package_specific_version(
        self, use_case, mock_registry_service, mock_storage_service, mock_logger
    ):
    ...
    @pytest.mark.asyncio
    async def test_block_package_malicious_package_fields(
        self, use_case, mock_registry_service, mock_storage_service, mock_logger
    ):
        ...
        assert (
            package.package_url == f"pkg:{ecosystem.lower()}/{package_name}@{version}"
        )
    ...
    @pytest.mark.asyncio
    async def test_block_package_registry_failure(
        self, use_case, mock_registry_service, mock_storage_service, mock_logger
    ):
    ...
    @pytest.mark.asyncio
    async def test_block_package_exception(
        self, use_case, mock_registry_service, mock_storage_service, mock_logger
    ):
    ...
    @pytest.mark.asyncio
    async def test_block_package_storage_exception_after_successful_block(
        self, use_case, mock_registry_service, mock_storage_service, mock_logger
    ):
    ...
    @pytest.mark.asyncio
    async def test_different_ecosystems_package_urls(
        self, use_case, mock_registry_service, mock_storage_service, mock_logger
    ):
    ...
    @pytest.mark.asyncio
    async def test_search_package_case_sensitive_ecosystem(
        self, use_case, mock_registry_service, mock_logger
    ):
        ...
        test_package = call_args[0]
        assert (
            test_package.package_url == f"pkg:pypi/{package_name}"
        )  # Lowercased in URL
    ...
    async def test_logging_calls(self, mock_registry_service, mock_storage_service):
        ...
        with patch(
            ...
            use_case = PackageManagementUseCase(
                mock_registry_service, mock_storage_service
            )
            package_name = "test-package"
            ecosystem = "npm"
            ...
            mock_logger.debug.assert_any_call(
                "Package search completed: 0 results found, blocked: False"
            )
            ...
            mock_logger.debug.assert_any_call(
                f"Blocking package: {package_name} ({ecosystem}) version 1.0.0"
            )
            mock_logger.debug.assert_any_call(f"Successfully blocked {package_name}")

    @pytest.mark.asyncio
    async def test_search_package_registry_service_none_during_cleanup(
        self, mock_logger
    ):
