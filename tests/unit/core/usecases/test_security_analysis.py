...
class TestSecurityAnalysisUseCase:
    ...
    def mock_packages_feed(self):
        ...
        feed.get_available_ecosystems = AsyncMock(return_value=["npm", "pypi", "maven"])
        return feed
    ...
    def mock_logger(self):
        ...
        with patch(
            "src.core.usecases.security_analysis.logging.getLogger"
        ) as mock_get_logger:
            mock_logger = Mock()
    ...
    @pytest.mark.asyncio
    async def test_crossref_analysis_no_malicious_packages(
        self, use_case, mock_packages_feed, mock_logger
    ):
        ...
        mock_packages_feed.fetch_malicious_packages.assert_called_once_with(
            max_packages=None, hours=hours, ecosystems=["npm"]
        )

    @pytest.mark.asyncio
    async def test_crossref_analysis_default_parameters(
        self, use_case, mock_packages_feed, mock_logger
    ):
        ...
        assert result["success"] is True
        mock_packages_feed.fetch_malicious_packages.assert_called_once_with(
            max_packages=None, hours=6, ecosystems=["npm", "pypi", "maven"]
        )

    @pytest.mark.asyncio
    async def test_crossref_analysis_custom_parameters(
        self, use_case, mock_packages_feed, mock_logger
    ):
        ...
        await use_case.crossref_analysis(hours, ecosystem, limit)

        mock_packages_feed.fetch_malicious_packages.assert_called_once_with(
            max_packages=limit, hours=hours, ecosystems=["pypi"]
        )

    @pytest.mark.asyncio
    async def test_crossref_analysis_ecosystem_filtering(
        self, use_case, mock_packages_feed, mock_logger
    ):
        ...
        npm_package = MaliciousPackage(
            ...
            ecosystem="npm",
            version="1.0.0",
            package_url="pkg:npm/npm-package@1.0.0",
            advisory_id="OSV-001",
            summary="",
            details="",
            aliases=[],
        ...
        pypi_package = MaliciousPackage(
            ...
            ecosystem="pypi",
            version="1.0.0",
            package_url="pkg:pypi/pypi-package@1.0.0",
            advisory_id="OSV-002",
            summary="",
            details="",
            aliases=[],
        ...
        # Mock the feed to return only npm packages when npm ecosystem is requested
        def mock_fetch_side_effect(max_packages=None, hours=None, ecosystems=None):
            all_packages = [npm_package, pypi_package]
            if ecosystems:
                return [pkg for pkg in all_packages if pkg.ecosystem in ecosystems]
            return all_packages

        mock_packages_feed.fetch_malicious_packages.side_effect = mock_fetch_side_effect
    ...
    @pytest.mark.asyncio
    async def test_crossref_analysis_case_insensitive_ecosystem_filtering(
        self, use_case, mock_packages_feed, mock_logger
    ):
    ...
    @pytest.mark.asyncio
    async def test_crossref_analysis_registry_unhealthy(
        self,
        use_case,
        mock_packages_feed,
    ...
    @pytest.mark.asyncio
    async def test_crossref_analysis_package_not_found_in_registry(
        self,
        use_case,
        mock_packages_feed,
        mock_registry_service,
    ...
    @pytest.mark.asyncio
    async def test_crossref_analysis_version_match_found(
        self, use_case, mock_packages_feed, mock_registry_service, mock_logger
    ):
    ...
    async def test_crossref_analysis_no_version_match(
        ...
        malicious_package = MaliciousPackage(
            ...
            ecosystem="npm",
            version="1.0.0",
            package_url="pkg:npm/package-name@1.0.0",
            advisory_id="OSV-001",
            summary="",
            details="",
            aliases=[],
    ...
    @pytest.mark.asyncio
    async def test_crossref_analysis_multiple_version_matches(
        self, use_case, mock_packages_feed, mock_registry_service, mock_logger
    ):
        ...
        malicious_package = MaliciousPackage(
            ...
            ecosystem="npm",
            version="1.0.0",
            package_url="pkg:npm/multi-match@1.0.0",
            advisory_id="OSV-001",
            summary="",
            details="",
            aliases=[],
    ...
    @pytest.mark.asyncio
    async def test_crossref_analysis_package_without_affected_versions(
        self, use_case, mock_packages_feed, mock_registry_service, mock_logger
    ):
        ...
        malicious_package = MaliciousPackage(
            ...
            ecosystem="npm",
            version="1.0.0",
            package_url="pkg:npm/single-version@1.0.0",
            advisory_id="OSV-001",
            summary="",
            details="",
            aliases=[],
    ...
    @pytest.mark.asyncio
    async def test_crossref_analysis_package_with_no_version_info(
        self, use_case, mock_packages_feed, mock_registry_service, mock_logger
    ):
        ...
        malicious_package = MaliciousPackage(
            ...
            ecosystem="npm",
            version=None,
            package_url="pkg:npm/no-version",
            advisory_id="OSV-001",
            summary="",
            details="",
            aliases=[],
    ...
    @pytest.mark.asyncio
    async def test_crossref_analysis_registry_results_without_version(
        self,
        use_case,
        mock_packages_feed,
        mock_registry_service,
        ...
        registry_name = mock_registry_service.get_registry_name()
        assert (
            safe_package[f"{registry_name.lower()}_versions"] == []
        )  # Empty because no valid versions

    @pytest.mark.asyncio
    async def test_crossref_analysis_search_exception(
        self,
        use_case,
        mock_packages_feed,
        mock_registry_service,
    ...
    @pytest.mark.asyncio
    async def test_crossref_analysis_multiple_packages_mixed_results(
        self, use_case, mock_packages_feed, mock_registry_service, mock_logger
    ):
        ...
        package1 = MaliciousPackage(
            ...
            ecosystem="npm",
            version="1.0.0",
            package_url="pkg:npm/match-pkg@1.0.0",
            advisory_id="OSV-001",
            summary="",
            details="",
            aliases=[],
        ...
        package3 = MaliciousPackage(
            ...
            ecosystem="npm",
            version="1.0.0",
            package_url="pkg:npm/not-found-pkg@1.0.0",
            advisory_id="OSV-003",
            summary="",
            details="",
            aliases=[],
    ...
    @pytest.mark.asyncio
    async def test_crossref_analysis_main_exception(
        self, use_case, mock_packages_feed, mock_registry_service, mock_logger
    ):
    ...
    @pytest.mark.asyncio
    async def test_crossref_analysis_cleanup_exception_ignored(
        self, use_case, mock_packages_feed, mock_registry_service, mock_logger
    ):
    ...
    @pytest.mark.asyncio
    async def test_crossref_analysis_registry_service_none_during_cleanup(
        self, use_case, mock_packages_feed, mock_logger
    ):
    ...
    @pytest.mark.asyncio
    async def test_crossref_analysis_logging_calls(
        self, mock_packages_feed, mock_registry_service
    ):
        ...
        with patch(
            ...
            use_case = SecurityAnalysisUseCase(
                mock_packages_feed, mock_registry_service
            )

            # Test with no packages
            await use_case.crossref_analysis(hours=12, ecosystem="pypi")

            # Check that the log message for "no packages found" is called
            info_calls = mock_logger.info.call_args_list
            assert any(
                "No malicious packages found in the last 12 hours for ecosystems: ['pypi']"
                in call[0][0]
                for call in info_calls
            ), f"Expected log message not found in calls: {[call[0][0] for call in info_calls]}"

            # Check debug calls for the starting message
            debug_calls = mock_logger.debug.call_args_list
            assert any(
                call[0][0].startswith(
                    "Starting security cross-reference analysis for pypi packages from last 12 hours"
                )
                for call in debug_calls
            ), f"Expected debug message not found in calls: {[call[0][0] for call in debug_calls]}"

    @pytest.mark.asyncio
    async def test_crossref_analysis_logging_with_results(
        self, mock_packages_feed, mock_registry_service, sample_malicious_package
    ):
        ...
        with patch(
            ...
            # Check debug level for the "Found packages to check" message
            mock_logger.debug.assert_any_call("Found 1 malicious packages to check")
            mock_logger.info.assert_any_call(
                "Cross-reference analysis complete: 0 critical matches, 0 safe packages, 1 not found, 0 errors"
            )

    @pytest.mark.asyncio
    async def test_crossref_analysis_warning_on_critical_match(
        self, mock_packages_feed, mock_registry_service
    ):
        ...
        with patch(
            ...
            malicious_package = MaliciousPackage(
                ...
                ecosystem="npm",
                version="1.0.0",
                package_url="pkg:npm/critical-pkg@1.0.0",
                advisory_id="OSV-001",
                summary="",
                details="",
                aliases=[],
    ...
    @pytest.mark.asyncio
    async def test_crossref_analysis_error_logging_for_package_exception(
        self, mock_packages_feed, mock_registry_service, sample_malicious_package
    ):
        ...
        with patch(
            ...
            mock_registry_service.search_packages.side_effect = Exception(
                "Search error"
            )
            ...
            mock_logger.error.assert_any_call(
                "Error checking package malicious-package (npm): Search error"
            )
