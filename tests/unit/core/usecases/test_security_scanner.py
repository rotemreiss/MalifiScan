...
class TestSecurityScanner:
    ...
    @pytest.mark.asyncio
    async def test_execute_scan_success_no_new_packages(
        self,
        security_scanner,
        mock_packages_feed,
        mock_packages_registry,
    ...
    @pytest.mark.asyncio
    async def test_execute_scan_success_with_new_packages(
        self,
        security_scanner,
        mock_packages_feed,
        mock_packages_registry,
        ...
        mock_packages_registry.check_existing_packages.return_value = []
        mock_packages_registry.block_packages.return_value = [
            sample_malicious_package.package_identifier
        ]
        ...
        # Verify mock calls
        mock_packages_registry.block_packages.assert_called_once_with(
            [sample_malicious_package]
        )

    @pytest.mark.asyncio
    async def test_execute_scan_feed_failure(
        self, security_scanner, mock_packages_feed
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_scan_registry_failure(
        self,
        security_scanner,
        mock_packages_feed,
        mock_packages_registry,
        ...
        mock_packages_registry.check_existing_packages.side_effect = Exception(
    ...
    @pytest.mark.asyncio
    async def test_execute_scan_storage_failure(
        self,
        security_scanner,
        mock_packages_feed,
        mock_packages_registry,
        ...
        mock_storage_service.store_scan_result.side_effect = Exception(
            "Storage unavailable"
        )
    ...
    @pytest.mark.asyncio
    async def test_notification_failure_does_not_fail_scan(
        self,
        security_scanner,
        mock_packages_feed,
        mock_packages_registry,
    ...
    @pytest.mark.asyncio
    async def test_execute_scan_empty_feed(
        self,
        security_scanner,
        mock_packages_feed,
        mock_packages_registry,
    ...
    @pytest.mark.asyncio
    async def test_execute_scan_mixed_scenarios(
        self,
        security_scanner,
        mock_packages_feed,
        mock_packages_registry,
    ...
    @pytest.mark.asyncio
    async def test_execute_scan_all_packages_already_exist(
        self,
        security_scanner,
        mock_packages_feed,
        mock_packages_registry,
    ...
    @pytest.mark.asyncio
    async def test_execute_scan_partial_blocking_failure(
        self,
        security_scanner,
        mock_packages_feed,
        mock_packages_registry,
    ...
    @pytest.mark.asyncio
    async def test_execute_scan_storage_service_both_operations_fail(
        self,
        security_scanner,
        mock_packages_feed,
        mock_packages_registry,
        ...
        mock_packages_registry.check_existing_packages.return_value = []
        mock_packages_registry.block_packages.return_value = [
            pkg.package_identifier for pkg in sample_malicious_packages
        ]

        # Both storage operations fail
        mock_storage_service.store_scan_result.side_effect = Exception(
            "Scan result storage failed"
        )
        mock_storage_service.store_malicious_packages.side_effect = Exception(
            "Package storage failed"
        )
    ...
    @pytest.mark.asyncio
    async def test_execute_scan_with_logging(
        self,
        security_scanner,
        mock_packages_feed,
        mock_packages_registry,
        ...
        with patch("src.core.usecases.security_scanner.logger") as mock_logger:
            ...
            mock_logger.info.assert_any_call(
                f"Found {len(sample_malicious_packages)} malicious packages in feed"
            )
            mock_logger.info.assert_any_call(
                "Checking for existing packages in registry"
            )
            mock_logger.info.assert_any_call(
                f"Blocking {len(sample_malicious_packages)} new malicious packages"
            )

    @pytest.mark.asyncio
    async def test_execute_scan_failed_scan_storage_attempt(
        self, security_scanner, mock_packages_feed, mock_storage_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_scan_failed_scan_storage_also_fails(
        self, security_scanner, mock_packages_feed, mock_storage_service
    ):
        ...
        with patch("src.core.usecases.security_scanner.logger") as mock_logger:
            ...
            mock_logger.error.assert_any_call(
                "Failed to store failed scan result: Storage completely down"
            )

    @pytest.mark.asyncio
    async def test_health_check_all_services_exception(
        self,
        security_scanner,
        mock_packages_feed,
        mock_packages_registry,
        ...
        mock_packages_feed.health_check.side_effect = Exception(
            "Feed service exception"
        )
        mock_packages_registry.health_check.side_effect = Exception(
            "Registry service exception"
        )
        mock_notification_service.health_check.side_effect = Exception(
            "Notification service exception"
        )
        mock_storage_service.health_check.side_effect = Exception(
        ...
        with patch("src.core.usecases.security_scanner.logger") as mock_logger:
            ...
            mock_logger.error.assert_any_call(
                "Packages feed health check failed: Feed service exception"
            )
            mock_logger.error.assert_any_call(
                "Registry service health check failed: Registry service exception"
            )
            mock_logger.error.assert_any_call(
                "Notification service health check failed: Notification service exception"
            )
            mock_logger.error.assert_any_call(
                "Storage service health check failed: Storage service exception"
            )

    @pytest.mark.asyncio
    async def test_scan_result_duration_calculation(
        self, security_scanner, mock_packages_feed
    ):
