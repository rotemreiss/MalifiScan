...
from src.core.usecases.scan_results import (
    DetailedScanResult,
    ScanResultsManager,
    ScanSummary,
)
...
def mock_registry_service():
    ...
    mock.get_registry_name = MagicMock(return_value="JFrog")
    return mock
...
class TestScanResultsManager:
    ...
    @pytest.mark.asyncio
    async def test_get_recent_scans_success(
        self, scan_results_manager, mock_storage_service, sample_scan_result
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_recent_scans_empty(
        self, scan_results_manager, mock_storage_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_recent_scans_storage_error(
        self, scan_results_manager, mock_storage_service
    ):
    ...
    async def test_get_scan_details_success(
        ...
        # Arrange
        mock_storage_service.get_scan_results.return_value = [sample_scan_result]
        scan_results_manager._get_findings_for_scan = AsyncMock(
            return_value=[sample_malicious_package]
        )
        ...
        mock_storage_service.get_scan_results.assert_called_once_with(
            scan_id="test-scan-123"
        )
    ...
    @pytest.mark.asyncio
    async def test_get_scan_details_storage_error(
        self, scan_results_manager, mock_storage_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_analyze_scan_results_with_findings(
        self, scan_results_manager, sample_scan_result, sample_malicious_package
    ):
        ...
        # Act
        result = await scan_results_manager._analyze_scan_results(
    ...
    @pytest.mark.asyncio
    async def test_analyze_scan_results_no_findings(
        self, scan_results_manager, sample_scan_result
    ):
...
class TestDetailedScanResult:
    ...
    def test_detailed_scan_result_creation(
        self, sample_scan_result, sample_malicious_package
    ):
