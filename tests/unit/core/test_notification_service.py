"""Unit tests for notification service interface and implementations."""
...
class TestNotificationServiceInterface:
    ...
    async def test_send_notification_success(
        self, mock_notification_service, sample_notification_event
    ):
        ...
        assert (
            mock_notification_service.sent_notifications[0] == sample_notification_event
        )
    ...
    async def test_send_multiple_notifications(
        self, mock_notification_service, sample_scan_result
    ):
    ...
    @pytest.mark.asyncio
    async def test_notification_event_different_levels(
        self, mock_notification_service, sample_scan_result
    ):
        ...
        sent_levels = [
            event.level for event in mock_notification_service.sent_notifications
        ]
        assert set(sent_levels) == set(levels)

    @pytest.mark.asyncio
    async def test_notification_event_different_channels(
        self, mock_notification_service, sample_scan_result
    ):
    ...
    async def test_interface_contract_compliance(
        ...
        send_result = await mock_notification_service.send_notification(
            sample_notification_event
        )
    ...
    async def test_concurrent_notifications(
        ...
        sent_event_ids = {
            event.event_id for event in mock_notification_service.sent_notifications
        }
    ...
    async def test_mixed_success_failure_scenarios(
        ...
        assert mock_notification_service.send_notification_call_count == 3
        assert (
            len(mock_notification_service.sent_notifications) == 2
    ...
    @pytest.mark.asyncio
    async def test_large_notification_payload(
        self, mock_notification_service, sample_scan_result
    ):
