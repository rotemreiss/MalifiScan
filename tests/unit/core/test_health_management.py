...
class TestHealthManagementUseCase:
    ...
    def mock_logger(self):
        ...
        with patch(
            "src.core.usecases.health_management.logging.getLogger"
        ) as mock_get_logger:
            mock_logger = Mock()
    ...
    async def test_get_service_health_status_empty_services(self, mock_logger):
        ...
        mock_logger.debug.assert_called_with(
            "Health check complete: 0/0 services healthy"
        )

    @pytest.mark.asyncio
    async def test_get_service_health_status_single_healthy_service(
        self, mock_logger, mock_healthy_service
    ):
        ...
        mock_logger.debug.assert_called_with(
            "Health check complete: 1/1 services healthy"
        )

    @pytest.mark.asyncio
    async def test_get_service_health_status_single_unhealthy_service(
        self, mock_logger, mock_unhealthy_service
    ):
        ...
        mock_logger.debug.assert_called_with(
            "Health check complete: 0/1 services healthy"
        )

    @pytest.mark.asyncio
    async def test_get_service_health_status_mixed_services(
        self, mock_logger, mock_healthy_service, mock_unhealthy_service
    ):
        """Test health check with mix of healthy and unhealthy services."""
        services = {"db": mock_healthy_service, "api": mock_unhealthy_service}
        use_case = HealthManagementUseCase(services)
        ...
        mock_unhealthy_service.health_check.assert_called_once()
        mock_logger.debug.assert_called_with(
            "Health check complete: 1/2 services healthy"
        )

    @pytest.mark.asyncio
    async def test_get_service_health_status_multiple_healthy_services(
        self, mock_logger
    ):
        ...
        services = {"database": service1, "cache": service2, "queue": service3}
        use_case = HealthManagementUseCase(services)
        ...
        assert all(
            service_status["healthy"] for service_status in result["services"].values()
        )
        ...
        mock_logger.debug.assert_called_with(
            "Health check complete: 3/3 services healthy"
        )

    @pytest.mark.asyncio
    async def test_get_service_health_status_service_exception(
        self, mock_logger, mock_service_with_exception
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_service_health_status_service_without_health_check(
        self, mock_logger, mock_service_without_health_check
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_service_health_status_mixed_with_exceptions(
        self, mock_logger, mock_healthy_service, mock_service_with_exception
    ):
    ...
    async def test_get_service_health_status_use_case_exception(self, mock_logger):
        ...
        # Mock logger.debug to raise an exception to trigger the outer catch block
        ...
        mock_logger.error.assert_called_with(
            "Error during health check: Logging failed"
        )

    @pytest.mark.asyncio
    async def test_get_service_health_status_service_returns_invalid_format(
        self, mock_logger
    ):
        ...
        assert result["total_count"] == 1
        assert (
            result["services"]["invalid_format_service"]["healthy"] == 0
        )  # Raw value stored
    ...
    async def test_get_service_health_status_large_number_of_services(
        ...
        mock_logger.debug.assert_called_with(
            "Health check complete: 7/10 services healthy"
        )
    ...
    async def test_get_service_health_status_concurrent_execution(self, mock_logger):
        ...
        services = {"slow1": slow_service1, "slow2": slow_service2}
