...
class TestCLIIntegration:
    ...
    def test_cli_help(self, cli_path, test_config_path):
        ...
        returncode, stdout, stderr = self.run_cli_command(
            cli_path, ["--help"], config_path=test_config_path
        )
    ...
    def test_cli_version(self, cli_path, test_config_path):
        ...
        returncode, stdout, stderr = self.run_cli_command(
            cli_path, ["--version"], config_path=test_config_path
        )
        ...
        if returncode == 0:
            ...
            for version_flag in ["-v", "version"]:
                returncode, stdout, stderr = self.run_cli_command(
                    cli_path, [version_flag], config_path=test_config_path
                )
                if returncode == 0:
    ...
    def test_cli_scan_command_exists(self, cli_path, test_config_path):
        ...
        if returncode == 0:
            ...
            for cmd in ["check", "analyze", "search"]:
                returncode, stdout, stderr = self.run_cli_command(
                    cli_path, [cmd, "--help"], config_path=test_config_path
                )
                if returncode == 0:
    ...
    def test_cli_scan_with_invalid_input(self, cli_path):
        ...
        returncode, stdout, stderr = self.run_cli_command(
            cli_path, ["scan", "thispackagedoesntexist123nonexistent"]
        )

        # This should either return error code or success with "not found" message
        if returncode != 0:
            logging.info(
                "✓ CLI scan command properly handles invalid packages (returns error)"
            )
        else:
            ...
            if any(
                phrase in output
                for phrase in ["not found", "no results", "not exist", "invalid"]
            ):
                logging.info(
                    "✓ CLI scan command properly handles invalid packages (returns not found)"
                )
            else:
    ...
    def test_cli_list_commands(self, cli_path):
        ...
        list_commands = [["list", "--help"], ["show", "--help"], ["ls", "--help"]]
...
"""
        ...
        try:
            ...
            returncode, stdout, stderr = self.run_cli_command(
                cli_path, ["--config", temp_config, "--help"]
            )
    ...
    def test_cli_comprehensive_functionality(self, cli_path):
        ...
        if help_works:
            ...
            logging.info(
                f"2. Available commands: {', '.join(available_commands) if available_commands else 'None detected'}"
            )
        else:
        ...
        returncode, stdout, stderr = self.run_cli_command(
            cli_path, ["nonexistent_command_xyz"]
        )
        error_handling = (
            returncode != 0 or "error" in stderr.lower() or "unknown" in stderr.lower()
        )
        ...
        for provider_test in provider_tests:
            ...
            for cmd in provider_test["commands"]:
                ...
                if returncode == 0:
                    ...
                    logging.info(
                        f"ℹ {' '.join(cmd)} exists but returned error: {stderr[:100]}"
                    )
                    break
            else:
                ...
                if returncode != -1:  # Command didn't timeout or fail to run
                    ...
                    if pkg["should_exist"]:
                        ...
                        if returncode == 0 or "found" in output or "result" in output:
                            logging.info(
                                f"✓ {pkg['name']}: {' '.join(cmd)} handled existing package correctly"
                            )
                        else:
                            logging.info(
                                f"ℹ {pkg['name']}: {' '.join(cmd)} returned {returncode}"
                            )
                    else:
                        ...
                        if (
                            ...
                            or "no result" in output
                            or "not exist" in output
                        ):
                            logging.info(
                                f"✓ {pkg['name']}: {' '.join(cmd)} handled non-existent package correctly"
                            )
                        else:
                            logging.info(
                                f"ℹ {pkg['name']}: {' '.join(cmd)} unexpected result for non-existent package"
                            )
    ...
    def test_cli_scan_crossref_with_memory_providers(self, cli_path, test_config_path):
        ...
        for scenario in test_scenarios:
            ...
            returncode, stdout, stderr = self.run_cli_command(
                cli_path, scenario["args"], config_path=test_config_path
            )
            ...
            if scenario["should_succeed"]:
                if returncode == 0:
                    ...
                    if any(
                        err in error_output
                        for err in ["config", "missing", "connection", "timeout"]
                    ):
                        logging.info(
                            f"ℹ {scenario['name']}: Command failed due to expected configuration issues: {stderr[:100]}"
                        )
                    else:
                        logging.warning(
                            f"⚠ {scenario['name']}: Unexpected command failure: {stderr[:100]}"
                        )
            else:
                # Command should fail
                if returncode != 0:
                    ...
                    logging.warning(
                        f"⚠ {scenario['name']}: Command succeeded but should have failed"
                    )
    ...
    def test_cli_scan_crossref_basic_functionality(self, cli_path, test_config_path):
        ...
        # Test crossref help
        returncode, stdout, stderr = self.run_cli_command(
            cli_path, ["scan", "crossref", "--help"], config_path=test_config_path
        )
        ...
        if returncode == 0:
            ...
            logging.info(
                f"✓ Crossref help works. Found options: {', '.join(found_options)}"
            )
        ...
        if returncode == 0:
            ...
            if any(
                err in error_output
                for err in ["config", "missing", "connection", "authentication"]
            ):
                logging.info(
                    "ℹ Crossref command failed due to configuration issues (may be expected)"
                )
            else:
