...
class TestJFrogRepositoryDiscovery:
    ...
    async def test_discover_unsupported_ecosystem(self, jfrog_registry):
        ...
        repos = await jfrog_registry.discover_repositories_by_ecosystem(
    ...
    async def test_ecosystem_package_type_mapping(self, jfrog_registry):
        ...
        assert not jfrog_registry._ecosystem_matches_package_type(
            "UnknownEcosystem", "unknown"
        )
    ...
    async def test_all_supported_ecosystems(self, jfrog_registry):
        ...
        total_ecosystems_with_repos = sum(
            1 for repos in ecosystem_results.values() if repos
        )
        logger.info(
            f"Found repositories for {total_ecosystems_with_repos} out of {len(supported_ecosystems)} ecosystems"
        )
