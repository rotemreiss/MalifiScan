...
class TestOSVFeedIntegration:
    ...
    async def test_fetch_malicious_packages_npm(self, osv_feed):
        ...
        malicious_packages = await osv_feed.fetch_malicious_packages(
            max_packages=10, hours=24 * 7  # Last 7 days
        )

        assert isinstance(
            malicious_packages, list
        ), "Should return a list of malicious packages"
        ...
        logging.info("OSV Feed Integration Test - NPM Logs:")
        ...
        if npm_packages:
            for i, pkg in enumerate(npm_packages[:3], 1):  # Log first 3 npm packages
                logging.info(
                    f"NPM Package {i}: {pkg.name} (Advisory ID: {pkg.advisory_id})"
                )
    ...
    async def test_fetch_npm_ecosystem_specific(self, osv_feed):
        ...
        try:
            ...
            logging.info(
                f"NPM-specific fetch: Found {len(npm_packages)} malicious npm packages"
            )
            ...
            for pkg in npm_packages:
                assert (
                    pkg.ecosystem == "npm"
                ), f"Expected npm package, got {pkg.ecosystem}"
    ...
    async def test_fetch_recent_packages_short_window(self, osv_feed):
        ...
        packages = await osv_feed.fetch_malicious_packages(max_packages=20, hours=24)
    ...
    async def test_error_handling_invalid_ecosystem(self, osv_feed):
        ...
        try:
            ...
            packages = await osv_feed._fetch_malicious_packages_for_ecosystem(
                ecosystem="invalid_ecosystem_that_does_not_exist", max_packages=1
            )

            # Should return empty list, not error
            assert isinstance(
                packages, list
            ), "Should return a list even for invalid ecosystem"
    ...
    async def test_comprehensive_npm_logging(self, osv_feed):
        ...
        try:
            ...
            logging.info(
                f"1. General fetch: {len(all_packages)} total, {len(npm_packages)} npm"
            )

            # Test 2: Recent packages (24 hours)
            recent_packages = await osv_feed.fetch_malicious_packages(
                max_packages=20, hours=24
            )
            recent_npm = [pkg for pkg in recent_packages if pkg.ecosystem == "npm"]

            logging.info(
                f"2. Recent (24h): {len(recent_packages)} total, {len(recent_npm)} npm"
            )

            # Test 3: Longer window (7 days)
            week_packages = await osv_feed.fetch_malicious_packages(
                max_packages=30, hours=24 * 7
            )
            week_npm = [pkg for pkg in week_packages if pkg.ecosystem == "npm"]

            logging.info(
                f"3. Week (7d): {len(week_packages)} total, {len(week_npm)} npm"
            )

            # Log details of npm packages found
            all_npm_found = (
                npm_packages
                + [p for p in recent_npm if p not in npm_packages]
                + [p for p in week_npm if p not in npm_packages and p not in recent_npm]
            )

            logging.info("\n=== NPM Packages Summary ===")
            logging.info(
                f"Unique NPM packages found: {len(set(pkg.advisory_id for pkg in all_npm_found if pkg.advisory_id))}"
            )
            ...
            if all_npm_found:
                ...
                for i, pkg in enumerate(all_npm_found[:5], 1):
                    logging.info(
                        f"  {i}. {pkg.name} ({pkg.advisory_id or 'No advisory ID'})"
                    )
                    summary = (
                        pkg.summary[:100] + "..."
                        if pkg.summary and len(pkg.summary) > 100
                        else pkg.summary or "No summary"
                    )
                    logging.info(f"     Summary: {summary}")
            else:
                logging.info(
                    "No npm packages found - this may be normal depending on timing"
                )
